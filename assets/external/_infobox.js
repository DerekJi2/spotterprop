function drawInfobox(baseUrl, category, infoboxContent, json, i){
    ...
    if(json.data[i].url)            { var url = json.data[i].url }
        else                        { url = 'Property/Detail/' + json.data[i].id }
    ...
    '<div class="infobox ' + color + '">' +
        '<div class="inner">' +
            '<div class="image">' +
                ...
                '<div class="overlay">' +
                    '<div class="wrapper">' +
                        '<a href="#" class="quick-view" data-toggle="modal" data-target="#modal" id="' + id + '">' + i18_get_QuickView() + '</a>' +
                        '<hr>' +
                        '<a href="' + baseUrl + LANG + "/" + url +  '" class="detail">' + i18_get_GoToDetail() + '</a>' +
                    '</div>' +
                '</div>' +
                '<a href="' + baseUrl + url +  '" class="description">' +
                    '<div class="meta">' +
                        price +
                        '<h2>' + title +  '</h2>' +
