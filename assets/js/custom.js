...
// Set width for inputs in horizontal search bar -----------------------------------------------------------------------
    ...
    // $( "#redefine-search-form" ).load( BASEURL + "assets/external/_search-bar.html", function() {
    //     setInputsWidth();
    //     //autoComplete();
    // });
...
//  iCheck -------------------------------------------------------------------------------------------------------------
    ...
    $('a').on('click', function (e) {
        ...
        if ( attr != undefined && attr != null && attr.indexOf('#') != 0 ) {
            e.preventDefault();
        ...
        else if ( attr == undefined || attr == null || attr == '#' ) {
            e.preventDefault();
        }
    });
...
function autoComplete(){
    if( !$("script[src='" + BASEURL + "assets/js/leaflet.js']").length ){
...
function drawOwlCarousel(_rtl){
    $.getScript( BASEURL + "assets/js/owl.carousel.min.js", function( data, textStatus, jqxhr ) {
        ...
        if( category == 'real_estate' ){
            if( json.data[a].item_specific ){
                if( json.data[a].item_specific.bedrooms ){
                    itemSpecific += '<span title="Bedrooms"><img src="' + BASEURL +'assets/img/bedrooms.png">' + json.data[a].item_specific.bedrooms + '</span>';
                }
                if( json.data[a].item_specific.bathrooms ){
                    itemSpecific += '<span title="Bathrooms"><img src="' + BASEURL +'assets/img/bathrooms.png">' + json.data[a].item_specific.bathrooms + '</span>';
                }
                if( json.data[a].item_specific.area ){
                    itemSpecific += '<span title="Area"><img src="' + BASEURL +'assets/img/area.png">' + json.data[a].item_specific.area + '<sup>2</sup></span>';
                }
                if( json.data[a].item_specific.garages ){
                    itemSpecific += '<span title="Garages"><img src="' + BASEURL +'assets/img/garages.png">' + json.data[a].item_specific.garages + '</span>';
                }
                return itemSpecific;
            }
        }
