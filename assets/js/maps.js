...
function createHomepageGoogleMap(_latitude,_longitude,json){
    ...
    function gMap(){
        ...
        for (var i = 0; i < json.data.length; i++) {
            ...
            if( json.data[i].featured == 1 ) {
                markerContent.innerHTML =
                    '<div class="map-marker featured' + color + '">' +
                        '<div class="icon">' +
                        '<img src="' + BASEURL + json.data[i].type_icon +  '">' +
                        '</div>' +
                    '</div>';
            }
            else {
            ...
            infoboxContent.innerHTML = drawInfobox(BASEURL, category, infoboxContent, json, i);
        ...
        google.maps.event.addListener(autocomplete, 'place_changed', function() {
            ...
            nsSearchPanel.OnGoogleLocationSelected(place.address_components);
            var address = '';
            if (place.address_components) {
                ...
                // Reload all properties: IMPORTANT
                gMap();
            }
        });
...
function createHomepageOSM(_latitude,_longitude,json,mapProvider){
    ...
    function osmMap(){
        ...
        for (var i = 0; i < json.data.length; i++) {
            ...
            if( json.data[i].type_icon ) var icon = '<img src="' + BASEURL + json.data[i].type_icon +  '">';
            else icon = '';
            ...
            var infoboxContent = document.createElement("div");
            marker.bindPopup(
                drawInfobox(BASEURL, category, infoboxContent, json, i)
            );
...
function itemDetailMap(json){
    ...
    var map = new google.maps.Map(mapElement, mapOptions);
    if( json.type_icon ) var icon = '<img src="' + BASEURL + json.type_icon +  '">';
    else icon = '';
...
function pushItemsToArray(json, a, category, visibleItemsArray){
    var itemPrice;
    var langTypeName = i18_get_Type(json.data[a].type);
    visibleItemsArray.push(
        '<li class="result-property-item-' + json.data[a].id + '">' +
            '<div class="item" id="' + json.data[a].id + '">' +
                '<a href="#" class="image">' +
                    '<div class="inner">' +
                        ...
                        '</div>' +
                        '<img src="' + BASEURL + json.data[a].gallery[0] + '" alt="">' +
                    '</div>' +
                '</a>' +
                '<div class="wrapper">' +
                    ...
                    '<div class="info">' +
                        '<div class="type">' +
                            '<i><img src="' + BASEURL + json.data[a].type_icon + '" alt=""></i>' +
                            '<span>' + langTypeName + '</span>' +
                        '</div>' +
